...
io.on("connection", async (socket: Socket) => {
  ...
  socket.on("chat:message", async (payload: { to?: string; text: string }) => {
    const from = username;
    console.log(
      `✉️  Msg from ${from}${payload.to ? ` to ${payload.to}` : ""}: "${
        payload.text
      }"`
    );
  ...
  socket.on("messages:get", async (targetUser?: string) => {
    let where = {};
    if (targetUser) {
      where = {
        OR: [
          { from: socket.data.user, to: targetUser },
          { from: targetUser, to: socket.data.user },
        ],
      };
    } else {
      where = { to: null }; // global chat only
    }
